...
// Mapeamento de cores do português para inglês
const cores = {
  'azul': 'blue',
  'vermelho': 'red',
  'verde': 'green',
...
recognition.onresult = (event) => {
  ...
  if (texto.includes('muda o fundo para')) {
    const corFalada = texto.split('muda o fundo para ')[1]?.trim();
    const cor = cores[corFalada];
    if (cor) {
      document.body.style.backgroundColor = cor;
      speak(`Fundo alterado para ${corFalada}`);
    } else {
      speak(`Cor "${corFalada}" não reconhecida.`);
    }
  } else if (texto.includes('muda a cor do título para')) {
    const corFalada = texto.split('muda a cor do título para ')[1]?.trim();
    const cor = cores[corFalada];
    if (cor) {
      document.querySelector('h1').style.color = cor;
      speak(`Cor do título alterada para ${corFalada}`);
    } else {
      speak(`Cor "${corFalada}" não reconhecida.`);
    }
